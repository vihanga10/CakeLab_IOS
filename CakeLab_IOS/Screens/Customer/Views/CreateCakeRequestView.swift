...
struct CreateCakeRequestView: View {
    ...
    @StateObject private var requestVM = CreateCakeRequestViewModel()
    ...
    @State private var categories = CakeRequestOptions.categories
    @State private var cakeStyles = CakeRequestOptions.cakeStyles
    @State private var dietaryOpts = CakeRequestOptions.dietaryOptions
    @State private var flavours = CakeRequestOptions.flavours
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                            ...
                            HStack(spacing: 16) {
                                PhotosPicker(selection: $refImages, maxSelectionCount: 5,
                                    ...
                                    ImagePickerButton(icon: "photo", label: "Gallery")
                                             ...
                                             .onChange(of: refImages) { _, newItems in
                                                 uploadPhotosToStorage(newItems)
                                             }
                        ...
                        VStack(spacing: 12) {
                            ...
                            .buttonStyle(.plain)
                            .disabled(requestVM.isSaving || actionInProgress)
    ...
    private func publishRequest() async {
        ...
        let customerProfile = await requestVM.fetchCurrentUserProfile(userID: userID)

        guard canPublishRequest else {
            let docID = initialDraft?.id ?? requestVM.newDocumentID(for: "draftRequests")
            let data = buildRequestData(documentID: docID, customerID: userID, customerProfile: customerProfile, status: "draft", timestampField: "savedAt")
            await requestVM.publishRequest(requestData: data, collection: "draftRequests", documentID: docID, cleanupDraftID: nil, triggerNotification: false, requestTitle: title, customerID: userID, notificationManager: notificationManager, onSuccess: { dismiss() })
            return
        }

        let docID = initialDraft?.id ?? requestVM.newDocumentID(for: "cakeRequests")
        let data = buildRequestData(documentID: docID, customerID: userID, customerProfile: customerProfile, status: "open", timestampField: "createdAt")
